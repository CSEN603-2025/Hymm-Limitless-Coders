...
const InternshipsForStudent = () => {
  ...
  const handleSearchClick = () => {
    const results = selectedInternships.filter((internship) => {
      ...
      const isAfterFrom = !fromDate || postedDate >= new Date(fromDate);
      const isBeforeTo = !toDate || postedDate <= new Date(toDate);
      const matchesPostedDate = isAfterFrom && isBeforeTo;

      return matchesSearch && matchesFilter && matchesPostedDate;
    });
    setFilteredInternships(results);
  };
  ...
  const handleDeleteReport = (id) => {
    const { [id]: _, ...rest } = reports;
    setReports(rest);
    localStorage.setItem(REPORT_STORAGE_KEY, JSON.stringify(rest));
  };
    ...
    <div className="internship-container" style={{ paddingTop: '200px' }}>
      ...
      <div className="search-filter-container">
        <input
          ...
          className="search-input"
        />
        <select value={statusFilter} onChange={handleFilterChange} className="status-filter">
          <option value="all">All</option>
          <option value="completed">Internship Completed</option>
          <option value="in-progress">Current Intership</option>
        </select>
      
  
        <div className="input-wrapper">
  <label className="floating-label" htmlFor="fromDate">From Date</label>
  <input
    id="fromDate"
    type="date"
    value={fromDate}
...
<div className="input-wrapper">
  <label className="floating-label" htmlFor="toDate">To Date</label>
  <input
    id="toDate"
    type="date"
    value={toDate}
